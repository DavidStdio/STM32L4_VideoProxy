...
//static uint8_t  USBD_VIDEO_OUT_Incplt (USBD_HandleTypeDef  *pdev);
//static uint8_t  USBD_VIDEO_IN_Incplt (USBD_HandleTypeDef  *pdev);
...
static uint8_t  *USBD_VIDEO_GetFSCfgDesc (/*uint8_t speed,*/ uint16_t *length);
static uint8_t  *USBD_VIDEO_GetOtherSpeedCfgDesc (/*uint8_t speed,*/ uint16_t *length);
static uint8_t  *USBD_VIDEO_GetDeviceQualifierDescriptor (/*uint8_t speed,*/ uint16_t *length);
...
USBD_ClassTypeDef  USBD_UVC =
{
  ...
  NULL,
  NULL ,
  NULL,//USBD_VIDEO_GetHSCfgDesc,
  USBD_VIDEO_GetFSCfgDesc,//USBD_VIDEO_GetFSCfgDesc,
...
static uint8_t usbd_video_CfgDescIso[] =
{
  /* Configuration 1 */
...
/* USB VIDEO device Configuration Descriptor */
static uint8_t usbd_video_CfgDescBulk[] =
{
  /* Configuration 1 */
  USB_CONFIGUARTION_DESC_SIZE,               // bLength                  9
  USB_DESC_TYPE_CONFIGURATION,               // bDescriptorType          2
...
#ifdef ISO
#define usbd_video_CfgDesc usbd_video_CfgDescIso
#else
#define usbd_video_CfgDesc usbd_video_CfgDescBulk
#endif
...
static uint8_t  USBD_VIDEO_DataIn (USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  static uint16_t last_jpeg_frame_size = sizeof frame;
...
{ 
  ...
  if (req->wLength)
  {
    /* Prepare the reception of the buffer over EP0 */
	  ...
	  {
		  //Probe Request
		  USBD_CtlPrepareRx (pdev, (uint8_t*)&video_control, req->wLength);
	  }
	  else if (req->wValue == 512)
	  {
		  //Commit Request
